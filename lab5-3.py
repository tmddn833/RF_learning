...
for i in range(trainSize):
    ...
    while not isDone:
        action = np.argmax(Q[currState, :] + np.random.randn(1, environment.action_space.n) / (i + 1))
        nextState, reward, isDone, probability = environment.step(action)
        Q[currState, action] = (1-learning_rate) * Q[currState, action]\
                               + learning_rate *(reward + discount * np.max(Q[nextState, :]))
        totalReward += reward
        currState = nextState
